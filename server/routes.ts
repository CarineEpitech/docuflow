...
- For questions about company documents, reference the folder and document names`;
  ...
  app.get("/api/crm/tags", isAuthenticated, async (req: any, res) => {
    try {
      const tags = await storage.getCrmTags();
      res.json(tags);
    } catch (error) {
  ...
  // Add a tag to a CRM project
  app.post("/api/crm/projects/:id/tags", isAuthenticated, async (req: any, res) => {
    try {
      const createSchema = z.object({
      ...
      const projectTag = await storage.addCrmProjectTag(req.params.id, parsed.data.tagId);
  ...
  // Remove a tag from a CRM project
  app.delete("/api/crm/projects/:id/tags/:tagId", isAuthenticated, async (req: any, res) => {
    try {
      await storage.removeCrmProjectTag(req.params.id, req.params.tagId);
      res.json({ success: true });
    } catch (error) {
