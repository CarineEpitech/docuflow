...
safeTool("list_projects", {
    description: "List all documentation projects. Returns project names, descriptions, and IDs.",
    inputSchema: {},
}, async () => {
...
safeTool("list_documents", {
    description: "List all documents/pages within a project. Returns document titles, IDs, and hierarchy.",
    inputSchema: {
        projectId: z.string().describe("The project UUID"),
    },
}, async ({ projectId }) => {
    const docs = await apiRequest("GET", `/api/projects/${projectId}/documents`);
    function formatDoc(doc, indent = 0) {
        const prefix = "  ".repeat(indent);
        let line = `${prefix}- ${doc.title || "Untitled"} (ID: ${doc.id})`;
        if (doc.children && doc.children.length > 0) {
            line +=
                "\n" +
                    doc.children.map((c) => formatDoc(c, indent + 1)).join("\n");
        }
        return line;
    }
    const formatted = docs.map((d) => formatDoc(d)).join("\n");
    return {
        content: [
...
safeTool("get_document", {
    description: "Get the full content of a document/page. Returns the title and readable text content.",
    inputSchema: {
...
safeTool("create_document", {
    ...
    const doc = await apiRequest("POST", `/api/projects/${projectId}/documents`, body);
    return {
        content: [
            {
                type: "text",
...
safeTool("update_document", {
    ...
    inputSchema: {
        ...
        content: z
            .string()
            .optional()
            .describe("New plain text content. Will be converted to a TipTap paragraph. (optional)"),
    },
...
safeTool("search", {
    description: "Search across all projects and documents by keyword. Returns matching document titles and snippets.",
    inputSchema: {
        query: z.string().describe("The search query text"),
    },
}, async ({ query }) => {
    const results = await apiRequest("GET", `/api/search?q=${encodeURIComponent(query)}`);
    ...
    const formatted = results
        .map((r) => `- ${r.title || "Untitled"} (ID: ${r.id}, Project: ${r.projectId})\n  Match: ${r.snippet || "N/A"}`)
        .join("\n");
    return {
        content: [
            {
...
safeTool("list_crm_projects", {
    description: "List all CRM projects with their status, type, dates, and associated project names.",
    inputSchema: {
        status: z
            .string()
            .optional()
            .describe("Filter by status (e.g. in_negotiation, won_in_progress, completed)"),
    },
}, async ({ status }) => {
...
safeTool("list_time_entries", {
    description: "List time tracking entries. Can filter by project, user, and date range.",
    inputSchema: {
        crmProjectId: z
            .string()
            .optional()
    ...
    if (crmProjectId)
        params.set("crmProjectId", crmProjectId);
    if (startDate)
        params.set("startDate", startDate);
    if (endDate)
        params.set("endDate", endDate);
    const qs = params.toString();
    const result = await apiRequest("GET", `/api/time-tracking/entries${qs ? `?${qs}` : ""}`);
...
safeTool("get_time_tracking_stats", {
    description: "Get time tracking statistics/summary. Shows total hours, breakdown by project.",
    inputSchema: {
        startDate: z
            .string()
            .optional()
    ...
    if (endDate)
        params.set("endDate", endDate);
    const qs = params.toString();
    const stats = await apiRequest("GET", `/api/time-tracking/stats${qs ? `?${qs}` : ""}`);
    return {
        content: [
            {
                type: "text",
...
safeTool("get_active_time_entry", {
    description: "Check if there is a currently active/running time tracking entry.",
    inputSchema: {},
}, async () => {
...
safeTool("ask_ai", {
    description: "Ask the AI assistant a question about your documentation. Uses semantic search across all your projects and documents to find relevant context.",
    inputSchema: {
...
safeTool("list_recent_documents", {
...
safeTool("list_users", {
    ...
    const formatted = (Array.isArray(users) ? users : [])
        .map((u) => `- ${u.firstName || ""} ${u.lastName || ""} (ID: ${u.id})\n  Email: ${u.email} | Role: ${u.role}`)
        .join("\n");
    return {
...
safeTool("get_notifications", {
    ...
    const formatted = notifications
        .map((n) => `- [${n.isRead ? "Read" : "Unread"}] ${n.title || "Notification"}\n  ${n.message || ""}\n  ${n.createdAt}`)
        .join("\n");
    return {
        content: [
